...
st.set_page_config(page_title="Buddy_AI", page_icon="💬", layout="centered")
st.title("🤖 Friendly AI")
...
if user_query:
    ...
    #1) Show = store user messages in UI state
    st.chat_message("user").markdown(user_query)
    st.session_state.chat_history.append({"role":"user", "content": user_query})

    #2) Persist tO DB (create a conv on first message, else append)
    if st.session_state.conversation_id is None:
        try:
            title = get_chat_title(selected_model, user_query) or "New Chat"
        except Exception:
            title = "New Chat"
        conv_id = create_new_conversations(title=title, role="user", content=user_query)
        st.session_state.conversation_id = conv_id
        st.session_state.conversation_title = title
    else:
        add_messages(st.session_state.conversation_id, "user", user_query)
    ...
    if st.session_state.conversation_id:
        add_messages(st.session_state.conversation_id, "assistant", assistant_text)
